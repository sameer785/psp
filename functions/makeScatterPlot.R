makeScatterPlot <- function(dat, lab){
  ...
  gp <- gp + th
  gp <- gp + geom_point(data=bgDat, color="gray60", shape=21, fill="gray60", size=pointSize, alpha=.1)
  ...
  gp <- gp + scale_size_manual(values=c(3, 3, 3), labels=lab)
  gp <- gp + scale_alpha_manual(values=c(1,1,1), labels=lab)
  gp <- gp + scale_color_manual(values=c('black', 'black', 'black'), labels=lab)
  gp <- gp + scale_fill_manual(values=c('orange1', 'slategray2', 'orchid'), labels=lab)
  gp <- gp + scale_shape_manual(values = c(21, 21, 23), labels=lab)
