...
setVars<-function(dat.xls){
 ...
 # dat.xls <- dat.xls %>% mutate(stressGranuleAssembly = ifelse(geneSymbol %in% granuleGenes, 1, 0))
  dat.xls <- dat.xls %>% mutate(glutSyn = ifelse(geneSymbol %in% glutSyn, 1, 0))
...
makeHetKoPlot <- function(dat.xls){
  ...
  scPlot <- makeScatterPlot(dat.curDat, lab)
  scPlot <- scPlot + xlab("Log2-fold change (KO/WT)") + ylab("Log2-fold change (Het/WT)")
  scPlot <- scPlot + geom_text_repel(aes(label=ifelse(((x < -fcCutOff | y < -fcCutOff)),
                                                      as.character(geneSymbol), '')), show.legend = FALSE, segment.color='black',
                                                      segment.size = .5, segment.alpha = .25)
    # scPlot <- scPlot + geom_text_repel(aes(label=ifelse(((color==1 | color==2) & (x< -5 | y< -5)), as.character(geneSymbol), '')), 
    #                                    show.legend = FALSE, 
    #                                    segment.color='black', 
    #                                    segment.size = .5, 
    #                                    segment.alpha = .25)
  
  scPlot <- scPlot + coord_cartesian(ylim=c(-5,0), xlim=c(-5,0))
  return(scPlot)
}

trio.hetko <- makeHetKoPlot(trio.xls)
ggsave(plot = trio.hetko, "~/Google Drive/psp/analysis/plots/trio/lfcScatter_ko_v_het_NEGATIVE.pdf", width = plotWidth, height = plotHeight, units = "in")

shank3.hetko <- makeHetKoPlot(shank3.xls)
ggsave(plot = shank3.hetko, "~/Google Drive/psp/analysis/plots/shank3/lfcScatter_ko_v_het_NEGAGIVE.pdf", width = plotWidth, height = plotHeight, units = "in")

grin2a.hetko <- makeHetKoPlot(grin2a.xls)
ggsave(plot = grin2a.hetko, "~/Google Drive/psp/analysis/plots/grin2a/lfcScatter_ko_v_het_NEGATIVE.pdf", width = plotWidth, height = plotHeight, units = "in")
...
goPathways2 <- "~/Google Drive/geneSets/mSigDb/v7_2/curated/c2.cp.kegg.v7.2.symbols.gmt"
getGoAnalyses <- function(dat){
  dat.koOverWt.go <- doGoAnalysis(dat, goPathways2, tStat=tst.koOverWt)
  dat.hetOverWt.go <- doGoAnalysis(dat, goPathways2, tStat=tst.hetOverWt)
  dat.koOverHet.go <- doGoAnalysis(dat, goPathways2, tStat=tst.koOverHet)
...
grin2a.go2 <- getGoAnalyses(grin2a.xls)
shank3.go2 <- getGoAnalyses(shank3.xls)
trio.go2 <- getGoAnalyses(trio.xls) 
```
...
grin2a.go <- setGoScatter(grin2a.go)
grin2a.go.scatter <- getGoScatter(grin2a.go)
grin2a.go.scatter <- grin2a.go.scatter + coord_cartesian(xlim=c(-3,0), ylim = c(-3,0))
ggsave(plot = grin2a.go.scatter, "~/Google Drive/psp/analysis/plots/grin2a/goScatter_koOverWt_v_hetOverWt_NEGATIVE.pdf", width = plotWidth, height = plotHeight, units = "in")

shank3.go <- setGoScatter(shank3.go)
shank3.go.scatter <- getGoScatter(shank3.go)
# ggsave(plot = shank3.go.scatter, "~/Google Drive/psp/analysis/plots/shank3/goScatter_koOverWt_v_hetOverWt.pdf", width = plotWidth, height = plotHeight, units = "in")
shank3.go.scatter <- shank3.go.scatter + coord_cartesian(xlim=c(-3, 0), ylim = c(-3,0))
ggsave(plot = shank3.go.scatter, "~/Google Drive/psp/analysis/plots/shank3/goScatter_koOverWt_v_hetOverWt_NEGATIVE.pdf", width = plotWidth, height = plotHeight, units = "in")

trio.go <- setGoScatter(trio.go)
trio.go.scatter <- getGoScatter(trio.go)
trio.go.scatter <- trio.go.scatter + coord_cartesian(xlim=c(-3,0), ylim = c(-3,0))
ggsave(plot = trio.go.scatter, "~/Google Drive/psp/analysis/plots/trio/goScatter_koOverWt_v_hetOverWt_NEGATIVE.pdf", width = plotWidth, height = plotHeight, units = "in")
```
...
sig.nes <- sig.go %>% select(pathway, NES.y.grin2a, NES.x.grin2a , NES.y.shank3,NES.x.shank3, NES.y, NES.x)
sig.nes <- sig.nes %>% rename(Grin2a_Het.over.WT =NES.y.grin2a, Grin2a_KO.over.WT =NES.x.grin2a, Shank3_Het.over.WT =NES.y.shank3, Shank3_KO.over.WT =NES.x.shank3, Trio_Het.over.WT =NES.y, Trio_KO.over.WT =NES.x)
...
col_fun(seq(-3, 3))
sig.nes.hmap <- Heatmap(sig.nes, row_names_gp = gpar(fontsize = 6), col=col_fun, heatmap_legend_param = list(title=NULL), cluster_columns = FALSE) #row_km=4
pdf(file = "~/Google Drive/psp/analysis/plots/combined/heatmap_anySigGo_nes_10e8.pdf", width = 5,  height = 40)
sig.nes.hmap
dev.off()
```
...
pVal <- 10e-3
all.go <- inner_join(grin2a.go2, shank3.go2, by="pathway", suffix=c(".grin2a", ".shank3"))
all.go <- inner_join(all.go, trio.go2, by="pathway")
sig.go <- all.go %>% filter(padj.x.grin2a < pVal | padj.x.shank3 < pVal | padj.x < pVal | padj.y.grin2a < pVal | padj.y.shank3 < pVal | padj.y < pVal)
sig.nes <- sig.go %>% select(pathway, NES.x.grin2a, NES.y.grin2a, NES.x.shank3, NES.y.shank3, NES.x, NES.y)
sig.nes <- sig.nes %>% rename(Grin2a_KO.over.WT =NES.x.grin2a, Grin2a_Het.over.WT =NES.y.grin2a, Shank3_KO.over.WT =NES.x.shank3, Shank3_Het.over.WT =NES.y.shank3, Trio_KO.over.WT =NES.x, Trio_Het.over.WT =NES.y)
sig.nes <- as.data.frame(sig.nes)
rownames(sig.nes) <- sig.nes$pathway
sig.nes <- sig.nes[,c(-1)]
library(circlize)
col_fun = colorRamp2(c(-3, 0, 3), c("blue", "white", "red"))
col_fun(seq(-3, 3))
sig.nes.hmap <- Heatmap(sig.nes, row_names_gp = gpar(fontsize = 4), col=col_fun, heatmap_legend_param = list(title=NULL), cluster_columns = FALSE)
pdf(file = "~/Google Drive/psp/analysis/plots/combined/heatmap_anySigGo_nes_KEGG.pdf", width = 4,  height = 12)
sig.nes.hmap
dev.off()
```
```{r}
...
col_fun = colorRamp2(c(-3, 0, 3), c("blue", "white", "red"))
col_fun(seq(-3, 3))
sig.nes.hmap <- Heatmap(sig.nes, row_names_gp = gpar(fontsize = 12), col=col_fun, heatmap_legend_param = list(title=NULL))
pdf(file = "~/Google Drive/psp/analysis/plots/combined/heatmap_allSigGo_nes.pdf", width = 10,  height = 10)
sig.nes.hmap
dev.off()
```
...
rna.grin2a.psd <- rna.grin2a.psd %>% mutate(y=log2FoldChange.ko, x=logFC.KO.over.WT)
...
rna.grin2a.psd <- rna.grin2a.psd %>% rowwise() %>% mutate(rnaPsd.both.sig=(sum(rna.ko.sig, psd.ko.sig, na.rm = TRUE)))
...
scPlot <- scPlot + ylab("Log2-fold change - RNA (KO/WT)") + xlab("Log2-fold change - PSD (KO/WT)")
scPlot <- scPlot + geom_text_repel(aes(label=ifelse((color==3), as.character(geneSymbol), '')), show.legend = FALSE, segment.color='black', segment.size = .5, segment.alpha = .25, size=fontSiz, max.overlaps = 20, nudge_y= 2,
                                     direction    = "x",
                                     vjust        = 1)
scPlot <- scPlot + coord_cartesian(ylim=yL, xlim=xL)
scPlot
...
scPlot <- scPlot + ylab("Log2-fold change - RNA (KO/WT)") + xlab("Log2-fold change - PSD (KO/WT)")
scPlot <- scPlot + geom_text_repel(aes(label=ifelse(((x>2 | y > .25) & (color==1 | color==2)), as.character(geneSymbol), '')), show.legend = FALSE, segment.color='black', segment.size = .5, segment.alpha = .25, size=fontSiz, nudge_x= 2,
                                     direction    = "y",
                                    )
scPlot <- scPlot + coord_cartesian(ylim=yL, xlim=xL)
scPlot
...
rna.grin2a.psd <- rna.grin2a.psd %>% mutate(y=log2FoldChange.het, x=logFC.Het.over.WT)
...
rna.grin2a.psd <- rna.grin2a.psd %>% mutate(rnaPsd.het.both.sig = as.factor(sum(rna.het.sig, psd.het.sig, na.rm = TRUE)))
...
scPlot <- scPlot + ylab("Log2-fold change - RNA (Het/WT)") + xlab("Log2-fold change - PSD (Het/WT)")
scPlot <- scPlot + geom_text_repel(aes(label=ifelse((color==3), as.character(geneSymbol), '')), show.legend = FALSE, segment.color='black', segment.size = .5, segment.alpha = .25, size=fontSiz)
scPlot <- scPlot + coord_cartesian(ylim=yL, xlim=xL)
scPlot
...
cNames[8] <- paste(cNames[8], ".trio", sep="")
cNames[9] <- paste(cNames[9], ".trio", sep="")
...
makeHetKoPlot <- function(dat.xls){
  fcCutOff <- 2
  dat.curDat <- getVars(dat.xls)
  lab <- c("", "Stress granule")
  scPlot <- makeScatterPlot(dat.curDat, lab)
  scPlot <- scPlot + xlab("Log2-fold change (KO/WT)") + ylab("Log2-fold change (Het/WT)")
  scPlot <- scPlot + geom_text_repel(aes(label=ifelse((((color==1 | color==2) & x> 1) | ((color==1 | color==2) & y > 1)) | (as.character(geneSymbol)=="G3bp1") | (as.character(geneSymbol)=="G3bp2"), as.character(geneSymbol), '')), show.legend = FALSE, segment.color='black', segment.size = .5, segment.alpha = .25, size=3)
  scPlot <- scPlot + coord_cartesian(ylim=c(-5,5), xlim=c(-5,5))
  return(scPlot)
}

trio.hetko <- makeHetKoPlot(trio.xls)
ggsave(plot = trio.hetko, "~/Google Drive/psp/analysis/plots/trio/lfcScatter_ko_v_het_stress_granule.pdf", width = plotWidth, height = plotHeight, units = "in")

shank3.hetko <- makeHetKoPlot(shank3.xls)
ggsave(plot = shank3.hetko, "~/Google Drive/psp/analysis/plots/shank3/lfcScatter_ko_v_het_stress_granule.pdf", width = plotWidth, height = plotHeight, units = "in")

grin2a.hetko <- makeHetKoPlot(grin2a.xls)
ggsave(plot = grin2a.hetko, "~/Google Drive/psp/analysis/plots/grin2a/lfcScatter_ko_v_het_stress_granule.pdf", width = plotWidth, height = plotHeight, units = "in")
...
getVars<-function(dat.xls){
  ...
  curDat <-  curDat %>% mutate(x=logFC.KO.over.WT, y=logFC.Het.over.WT, color=glutSyn)
  return(curDat)
}
...
makeHetKoPlot <- function(dat.xls){
  fcCutOff <- 2
  dat.curDat <- getVars(dat.xls)
  lab <- c("", "Glutamatergic synapse")
  scPlot <- makeScatterPlot(dat.curDat, lab)
  scPlot <- scPlot + xlab("Log2-fold change (KO/WT)") + ylab("Log2-fold change (Het/WT)")
  scPlot <- scPlot + geom_text_repel(aes(label=ifelse(((color==1 & x< -2) | (color==1 & y < -2) | (color==1 & y > 2) | (color==1 & x > 2)), as.character(geneSymbol), '')), show.legend = FALSE, segment.color='black', segment.size = .5, segment.alpha = .25, size=3)
  scPlot <- scPlot + coord_cartesian(ylim=c(-5,5), xlim=c(-5,5))
  return(scPlot)
}

trio.hetko <- makeHetKoPlot(trio.xls)
ggsave(plot = trio.hetko, "~/Google Drive/psp/analysis/plots/trio/lfcScatter_ko_v_het_glutamatergic_synapse.pdf", width = plotWidth, height = plotHeight, units = "in")

shank3.hetko <- makeHetKoPlot(shank3.xls)
ggsave(plot = shank3.hetko, "~/Google Drive/psp/analysis/plots/shank3/lfcScatter_ko_v_het_glutamatergic_synapse.pdf", width = plotWidth, height = plotHeight, units = "in")

grin2a.hetko <- makeHetKoPlot(grin2a.xls)
ggsave(plot = grin2a.hetko, "~/Google Drive/psp/analysis/plots/grin2a/lfcScatter_ko_v_het_glutamatergic_synapse.pdf", width = plotWidth, height = plotHeight, units = "in")
...
getVars<-function(dat.xls){
  ...
  curDat <-  curDat %>% mutate(x=logFC.KO.over.WT, y=logFC.Het.over.WT, color=vgc)
  return(curDat)
}
...
trio.hetko <- makeHetKoPlot(trio.xls)
ggsave(plot = trio.hetko, "~/Google Drive/psp/analysis/plots/trio/lfcScatter_ko_v_het_voltage_gated_ion_channel.pdf", width = plotWidth, height = plotHeight, units = "in")

shank3.hetko <- makeHetKoPlot(shank3.xls)
ggsave(plot = shank3.hetko, "~/Google Drive/psp/analysis/plots/shank3/lfcScatter_ko_v_het_voltage_gated_ion_channel.pdf", width = plotWidth, height = plotHeight, units = "in")

grin2a.hetko <- makeHetKoPlot(grin2a.xls)
ggsave(plot = grin2a.hetko, "~/Google Drive/psp/analysis/plots/grin2a/lfcScatter_ko_v_het_voltage_gated_ion_channel.pdf", width = plotWidth, height = plotHeight, units = "in")
...
setGoScatter <- function(dat.go){
  pVal = 10e-3
  dat.go <- dat.go %>% mutate(trio.sig = ifelse(padj.x < pVal, 1, 0))
  dat.go <- dat.go %>% mutate(shank3.sig = ifelse(padj.x.shank3 < pVal, 2, 0))
  dat.go <- dat.go %>% mutate(both.sig = as.factor(trio.sig + shank3.sig))
  dat.go <- dat.go %>% mutate(X=NES.x, Y=NES.x.shank3)
  dat.go <- dat.go %>% mutate(color=both.sig)
  return (dat.go)
}
...
getGoScatter <- function(dat.go){
  sa <- dat.go %>% select(pathway, X, Y, color)
  sa <- na.omit(sa)
  lab  <- c("NS", "Sig. Trio (KO/WT)", "Sig. Shank3 (KO/WT)", "Sig. Both")
  koOverWtVhetOverWt.go.plot <- plotGoScatter(sa, lab)
  koOverWtVhetOverWt.go.plot <- koOverWtVhetOverWt.go.plot + xlab("Normalized Enrichment Score - Trio - (KO/WT)") + ylab("Normalized Enrichment Score - Shank3 - (KO/WT)")
  return(koOverWtVhetOverWt.go.plot)
}
...
all.go <- inner_join(grin2a.go, shank3.go, by="pathway", suffix=c(".grin2a", ".shank3"))
all.go <- inner_join(all.go, trio.go, by="pathway")
trioVshank3.go <- getGoScatter(setGoScatter(all.go))
trioVshank3.go
ggsave(plot = trioVshank3.go, "~/Google Drive/psp/analysis/plots/combined/goScatter_trio_ko_v_shank3_ko_DIVERGENT.pdf", width = plotWidth, height = plotHeight, units = "in")

```

```{r}
require(circlize)
makeIndividualGoHeatmap <- function(goDat, pVal){
  
  go.neg <- goDat %>% filter(NES.x < 0 | NES.y < 0)
  go.neg.anySig <- go.neg %>% filter(padj.x < padjPval | padj.y < padjPval)
  dim(go.neg.anySig)
  
  go.sig.nes <- go.neg.anySig %>% select(pathway, NES, NES.y, NES.x)
  go.sig.nes <- go.sig.nes %>% rename(koOverHet=NES, hetOverWt=NES.y, koOverWt=NES.x)
  
  go.sig.nes <- as.data.frame(go.sig.nes)
  rownames(go.sig.nes) <- substring(go.sig.nes$pathway,4)
  go.sig.nes <- go.sig.nes[,c(-1)]
  col_fun = colorRamp2(c(-3, 0, 3), c("blue", "white", "red"))
  go.sig.nes.hmap <- Heatmap(go.sig.nes, row_names_gp = gpar(fontsize = 6), col=col_fun, heatmap_legend_param = list(title=NULL), cluster_columns = FALSE, column_labels = c("KO/Het", "Het/WT", "KO/WT"))
  return(go.sig.nes.hmap)
}

padjPval <- 10e-3
grin2a.go.sig.nes.hmap <- makeIndividualGoHeatmap(grin2a.go, padjPval)
pdf(file = "~/Google Drive/psp/analysis/plots/grin2a/heatmap_allSigGo_E3_nes.pdf", width = 5,  height = 15)
grin2a.go.sig.nes.hmap
dev.off()

padjPval <- 10e-6
grin2a.go.sig.nes.hmap <- makeIndividualGoHeatmap(shank3.go, padjPval)
pdf(file = "~/Google Drive/psp/analysis/plots/shank3/heatmap_allSigGo_E3_nes.pdf", width = 5,  height = 15)
grin2a.go.sig.nes.hmap
dev.off()

padjPval <- 10e-6
grin2a.go.sig.nes.hmap <- makeIndividualGoHeatmap(trio.go, padjPval)
pdf(file = "~/Google Drive/psp/analysis/plots/trio/heatmap_allSigGo_E3_nes.pdf", width = 5,  height = 15)
grin2a.go.sig.nes.hmap
dev.off()
