...
Gene ontologies associated with glutamatergic function are downregulated even when omitting Shank3. 


Plotting genes associated with RNA splicing GO

```{r}
subsetGenes <- str_to_title(shank3.go$up$x$data$leadingEdge[[3]]) ### Translation initiaton
...
```{r}
subsetGenes <- str_to_title(shank3.go$up$x$data$leadingEdge[[13]]) ### Translation initiaton
subsetGenes.sample <- sample(subsetGenes, min(length(subsetGenes), size = 30))
...
```{r}
subsetGenes <- str_to_title(shank3.go$down$x$data$leadingEdge[[1]]) ### Translation initiaton
subsetGenes.sample <- sample(subsetGenes, min(length(subsetGenes), size = 30))
